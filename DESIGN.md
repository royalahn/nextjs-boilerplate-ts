...
Pretendard is the primary sans-serif for the product UI, giving the app a clean, compact, and highly legible Korean-friendly surface. Cascadia Mono is the fixed-width companion for code-like labels, counters, timestamps, and technical metadata, while Font Awesome 6.5.1 provides the shared icon library for navigation and utility controls. Black-pill primary buttons (`{rounded.full}`) dominate marketing CTAs; the brand color, signature canary yellow ({colors.brand-yellow}), is reserved for the wordmark, top promo banners, and "yellow tag" feature pills — never as a primary CTA. The 4-tier pricing comparison (Free / Starter / Business / Enterprise) leads into the densest surface in the system: a feature comparison table that runs ~80 rows deep across multiple section dividers.
...
**Pretendard** (primary): The app's main sans-serif typeface. Used across every UI surface from oversized hero displays to micro labels. It keeps the interface clean, highly legible, and comfortable for Korean/Latin mixed content. Fallbacks: Noto Sans KR, -apple-system, BlinkMacSystemFont, sans-serif.

**Cascadia Mono** (secondary): The fixed-width companion for code snippets, timestamps, IDs, counts, and technical metadata. Fallbacks: Pretendard, monospace.

**Font Awesome 6.5.1** (icon system): The shared icon source for navigation, actions, and utility affordances. Use a consistent icon weight and keep icons visually quiet next to the text.
